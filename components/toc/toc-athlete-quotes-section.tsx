...
export function TocAthleteQuotesSection() {
  return (
    <section id="voices" className={`relative scroll-mt-20 bg-[#0B1D3A] text-white ${tocSectionClass()}`}>
      ...
      <div className="container mx-auto w-full px-4 sm:px-6 max-w-6xl pt-4">
        <div className="max-w-3xl mb-8 sm:mb-10">
          ...
          <p className="text-white/75 text-base sm:text-lg leading-relaxed">{TOC_ATHLETE_QUOTES.lead}</p>
        </div>

        <ul className="grid grid-cols-1 gap-4 sm:gap-5 md:grid-cols-2 list-none p-0 m-0">
          {TOC_ATHLETE_QUOTES.quotes.map(({ name, credentials, quote }) => (
            <li
              key={name}
              className="flex h-full flex-col rounded-sm border-2 border-white/10 bg-[#060f1f]/60 p-5 sm:p-6 md:p-7 border-l-4 border-l-[#CC0000]"
            >
