...
**Event workspace ↔ forum sync (NHSCA Duals, etc.):** For event-linked threads (`context_type = 'event'`, `context_id = event_slug`), workspace and forum stay in sync. **Workspace** = who can see the event hub (roster, chat). **Workspace members** = (1) distinct `parent_user_id` from `national_team_event_registrations` (paid, backfilled when parent loads hub), (2) rows in `event_workspace_members` (explicit adds: family, kids, or joined via forum invite). **Sync:** On hub load, all workspace members are added to the event thread; all thread members are added to `event_workspace_members` (source `forum_invite`) so they see the hub. **Add family/kid:** Workspace members can POST to `/api/national-team/workspace/[eventSlug]/members` with `{ email }`; the user is added to `event_workspace_members` and to the event thread. **Join via invite link** or **Add member** in the thread UI also adds the user to `event_workspace_members` so they get hub access. Schema: `scripts/event-workspace-members-and-sync.md` (run in Supabase SQL Editor).
