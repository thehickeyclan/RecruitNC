...
| **4** | **STATES (NCHSAA)** | **`/nchsaa`, `/nchsaa/2026`, `/nchsaa/2025`, `/nchsaa/archive`** | **Full plan: `docs/STATES-TAB-MIGRATION-TO-RECRUITNC.md`** — Phase 1: Overview + `lib/regional-data.ts`. Phase 2: [year] results + TournamentBracketModal. Phase 3: Archive + StateChampionsTabs. Browser Supabase only; RLS for wrestling_nchsaa_results, most_outstanding_wrestlers, tournament_champions. |
...
Use **`docs/STATES-TAB-MIGRATION-TO-RECRUITNC.md`** for the full STATES (NCHSAA) migration (Phase 4).
