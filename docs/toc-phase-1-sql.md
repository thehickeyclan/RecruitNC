...
create table if not exists public.toc_media_requests (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists idx_toc_media_status on public.toc_media_requests(status, created_at desc);
...
alter table public.toc_media_requests enable row level security;
...
drop policy if exists "Service role full toc_media_requests" on public.toc_media_requests;
create policy "Service role full toc_media_requests"
  on public.toc_media_requests for all to service_role using (true) with check (true);
```
