...
export default function DebugStatsFixedPage() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const response = await fetch("/api/debug-stats-fixed")
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="bg-gray-100">
            ...
            <th className="border p-2 text-left">Overridden?</th>
          </tr>
        </thead>
        <tbody>
          {athletes.map((athlete) => (
            <tr key={athlete.id} className="hover:bg-gray-50">
              ...
              <td className="border p-2">{athlete.wasOverridden ? "Yes" : "No"}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="mb-6 text-2xl font-bold">Debug Stats (Fixed)</h1>
