...
export const handleMostStateChampionsBySchool: QueryHandler = async (
  params,
  request,
  messageId
) => {
  ...
  // Query all state champions (place = 1)
  ...
  if (error) {
    console.error("[Handler] most_state_champions_by_school error:", error)
    throw error
  }
  ...
  const schoolCounts = Object.entries(bySchool)
    .map(([school, names]) => ({
      school,
      count: names.size,
      totalChampionships: data?.filter((r: any) => r.school === school).length || 0,
    }))
    .sort((a, b) => {
      // Sort by unique wrestler count first, then by total championships
      if (b.count !== a.count) {
  ...
  return {
    aggregateResult: {
      schoolCounts,
      type: "most_state_champions_by_school",
    },
  }
}
