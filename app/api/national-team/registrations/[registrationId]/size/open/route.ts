...
/**
 * PATCH: Update gear sizes — no auth. For use with the unpublished open hub link only.
 * Anyone with the link can update any registration. Use at your own risk.
 */
export async function PATCH(
  ...
  const row = reg as { status: string }
  if (row.status !== "paid") {
  ...
  if (updateErr) {
    console.error("[national-team/registrations/size/open] update error:", updateErr)
