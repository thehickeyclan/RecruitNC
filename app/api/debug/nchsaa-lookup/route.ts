 ...
 * - profile_style: result of getNCHSAAResultsForProfile(name) — same as unified profile / rankings
 ...
 *
 * If profile_style is empty but raw_by_last_name has rows, set the athlete's wrestling_name to the exact
 * wrestler_name from one of those rows (or add an alias) so the profile can match.
 */
  ...
  try {
    ...
    if (!name) {
      return NextResponse.json(
        { error: "Missing name. Use ?name=First+Last and optional &year=2026" },
        { status: 400 },
      )
    }
