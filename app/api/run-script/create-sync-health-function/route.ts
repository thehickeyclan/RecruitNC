...
-- Creates a Postgres function that returns a JSONB report with:
-- - headline: basic coverage counts
-- - coverage: matched coverage against aliases
-- - non_canonical: athletes with non-standard division values
-- - mismatches: athletes whose division != mapped division
-- - top_unmatched: normalized college names not matched by any mapping
...
raw_mappings AS (
  SELECT
    ...
    COALESCE(aliases::text, '') AS aliases_text
  FROM logo_mappings
...
export async function POST() {
  ...
  if (!connectionString) {
    return NextResponse.json({ ok: false, error: "POSTGRES_URL is not configured" }, { status: 500 })
  }
  ...
  try {
    ...
    return NextResponse.json({ ok: true, message: "division_sync_health_report created/updated" })
  } catch (err: any) {
    console.error("Create health function failed:", err)
