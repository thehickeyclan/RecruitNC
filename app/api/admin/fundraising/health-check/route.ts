...
export async function GET() {
  ...
  try {
    ...
    // Step 4: Get EXISTING parent-athlete links (the working data model)
    const { data: parentLinks, error: parentLinksError } = await supabase
      .from("parent_athlete_links")
      .select("athlete_id, user_id")
    
    if (parentLinksError) {
      console.error("[v0] Error fetching parent links:", parentLinksError)
    }

    // Step 5: Get athletes with claimed_by_user_id set
    const { data: claimedAthletes, error: claimedError } = await supabase
      .from("athletes")
      .select("id, claimed_by_user_id")
      .not("claimed_by_user_id", "is", null)
    
    if (claimedError) {
      console.error("[v0] Error fetching claimed athletes:", claimedError)
    }
    ...
    for (const athlete of athletes || []) {
      ...
      // Critical: Has donations but no parent linked (money can't be managed)
      if (totalRaised > 0 && !hasParent) {
        issues.push({
          type: "has_donations_no_parent",
          severity: "critical",
          message: `Has $${(totalRaised / 100).toLocaleString()} raised but no parent account linked`
        })
      }

      // Warning: No parent linked (no donations yet)
      if (!hasParent && totalRaised === 0 && hasPage) {
        issues.push({
          type: "no_parent",
          severity: "warning",
          message: "No parent account linked - they won't be able to manage wallet"
        })
      }
