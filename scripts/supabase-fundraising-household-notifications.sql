-- Run in Supabase SQL Editor (production): gift-page + activation household alerts.
-- Email + SMS defaults ON for donation and activation texts; parents turn off under Profile → Fundraising notifications.
...
alter table public.user_profiles
  add column if not exists notify_sms_fundraising_activation boolean not null default true;
...
comment on column public.user_profiles.notify_sms_fundraising_activation is
  'When true and cell_phone is set, SMS when staff approves fundraising page activation (default on; turn off in profile).';
...
-- New-signup defaults for deployments that already added columns with older defaults:
alter table public.user_profiles
  alter column notify_sms_fundraising_gifts set default true;

alter table public.user_profiles
  alter column notify_sms_fundraising_activation set default true;
