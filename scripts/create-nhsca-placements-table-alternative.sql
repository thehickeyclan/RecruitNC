...
-- ALTERNATIVE VERSION: Without pg_trgm extension (if extension can't be enabled)
...
-- Simple text index for name matching (no extension required)
CREATE INDEX IF NOT EXISTS idx_nhsca_placements_name_lower ON nhsca_placements(LOWER(athlete_name));
