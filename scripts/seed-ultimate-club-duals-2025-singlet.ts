/**
 * Create Ultimate Club Duals 2025 singlet — sold out, $75, public store listing.
 *
 * Usage: npx tsx scripts/seed-ultimate-club-duals-2025-singlet.ts
 * Requires .env.local: Supabase service role + BLOB_READ_WRITE_TOKEN
 */
...
const SLUG = "ultimate-club-duals-2025-singlet"
const SOURCE_IMAGE = "public/images/store/ultimate-club-duals-2025-singlet-source.png"
const PRICE = 75
const SKU_PREFIX = "NCU-UC25-SING"
...
async function main() {
  ...
  const productRow = {
    name: "Ultimate Club Duals 2025 Singlet",
    description:
      "Official NC United singlet from Ultimate Club Duals 2025. Takedown competition cut with North Carolina script front, NC crest, and navy trim. This season is complete — listing kept for reference; all sizes are sold out.",
  ...
  const variantRows = NHSCA_HUB_GEAR_SIZES.flatMap((size) => ({
    product_id: productId,
    color: "White",
    size,
    sku: `${SKU_PREFIX}-WHT-${size.replace(/\s+/g, "")}`,
    stock_quantity: 0,
  }))
