...
const IMAGES = [
  "public/images/toc/toc-2026-official-tee-front.png",
  "public/images/toc/toc-2026-official-tee-back.png",
]
...
async function main() {
  ...
  if (existing?.id) {
    ...
    const { data, error } = await admin.from("products").insert(productRow).select("id").single()
  ...
  const { error: deleteVariantsError } = await admin.from("product_variants").delete().eq("product_id", productId)
  ...
  console.log(`Product URL: https://app.ncwrestlingunited.com/store-app/product/${productId}`)
  console.log(`Price: $${PRICE}; sizes: ${SIZES.join(", ")}; fulfillment: TOC pickup`)
