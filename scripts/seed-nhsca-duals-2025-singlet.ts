/**
 * Create NHSCA Duals 2025 singlet — sold out, $75, public store listing.
 *
 * Usage: npx tsx scripts/seed-nhsca-duals-2025-singlet.ts
...
const SLUG = "nhsca-duals-2025-singlet"
const SOURCE_IMAGE = "public/images/store/nhsca-duals-2025-singlet-source.png"
const PRICE = 75
const SKU_PREFIX = "NCU-NH25-SING"
...
async function main() {
  ...
  const productRow = {
    name: "NHSCA Duals 2025 Singlet",
    description:
      "Official NC United singlet from NHSCA Duals 2025. Takedown competition cut — white top with North Carolina United chest graphic, black lower body, and red accent stripes. This season is complete — listing kept for reference; all sizes are sold out.",
  ...
  const variantRows = NHSCA_HUB_GEAR_SIZES.map((size) => ({
    ...
    color: "Black",
    size,
    sku: `${SKU_PREFIX}-BLK-${size.replace(/\s+/g, "")}`,
    stock_quantity: 0,
  }))
