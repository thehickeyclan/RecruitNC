...
**NCHSAA dual team (state duals):** For "show dual team state champions", "NCHSAA dual team", "state duals winners", "who won duals in [year]", or **which school has the most dual team titles**, call \`nchsaa_dual_team_champions\`. Use \`leaderboard: true\` for most-titles / school ranking questions. Pass optional \`year\`, \`division\`, or \`school\` when the user narrows the question. These rows are **NCHSAA state** dual championships — not NHSCA national duals; if the user only says NHSCA duals, clarify, but for NC state duals always use this tool rather than guessing.
