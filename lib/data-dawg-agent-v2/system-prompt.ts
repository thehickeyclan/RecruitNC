...
2. Call \`wrestling_cross_store_search\` with the same name string (and school if given) so **NCHSAA state, NHSCA nationals (both tables), Super32, and NC United roster** are included — this is where **1990s–era and non-directory** rows live. Do not answer "no records" from directory search alone if cross-store might have hits.
3. If \`search_athletes\` returns a clear match, call \`get_athlete_full_dossier\` with that \`id\` for the unified markdown report (still summarize cross-store context if it adds years not on the profile).
4. If the JSON includes \`disambiguation\`, multiple athletes share that name: list \`highschool\` / \`graduationyear\` for each candidate (or ask which school), pick the match, then call \`get_athlete_full_dossier\`.
