...
export function tocDrawToWinnersBracketTree(draw: TocBracketDraw): BracketTreeDisplay {
  ...
  const finalBout = draw.bouts.find((b) => b.roundLabel === "Winners final")
...
/** Consolation bracket bouts grouped into rounds for generic tree renderer. */
export function tocDrawToConsolationBracketTree(draw: TocBracketDraw): BracketTreeDisplay | null {
  ...
  const consiR2 = draw.bouts.filter((b) => b.side === "losers" && b.roundLabel === "Consolation R2").sort((a, b) => a.boutNumber - b.boutNumber)
  const consiSf = draw.bouts.filter((b) => b.side === "losers" && b.roundLabel === "Consolation semifinals").sort((a, b) => a.boutNumber - b.boutNumber)
  const consiF = draw.bouts.find((b) => b.side === "losers" && b.roundLabel === "Consolation final")

  if (consiR1.length === 0) return null

  const rounds = [
    consiR1.map((b, i) => boutToMatch(b, 0, i, participantById)),
    consiR2.map((b, i) => boutToMatch(b, 1, i, participantById)),
    consiSf.map((b, i) => boutToMatch(b, 2, i, participantById)),
    consiF ? [boutToMatch(consiF, 3, 0, participantById)] : [],
  ].filter((r) => r.length > 0)
