...
describe("public rankings cap", () => {
  it("publishes top 20 only for active public class years", () => {
    expect(PUBLISHED_PUBLIC_RANKINGS_YEARS).toEqual([2027, 2028])
    expect(PUBLIC_RANKINGS_MAX_BY_YEAR[2026]).toBeUndefined()
    expect(PUBLIC_RANKINGS_MAX_BY_YEAR[2027]).toBe(20)
    expect(PUBLIC_RANKINGS_MAX_BY_YEAR[2028]).toBe(20)
  ...
  it("clamps 'all' and oversized topN to top 20", () => {
    expect(clampProspectRankingsLimit(2027, null)).toBe(20)
    expect(clampProspectRankingsLimit(2027, 1000)).toBe(20)
    expect(clampProspectRankingsLimit(2027, 10)).toBe(10)
    expect(clampProspectRankingsLimit(2028, 50)).toBe(20)
  })

  it("defaults unknown years to 20", () => {
    expect(getPublicRankingsMax(2031)).toBe(20)
  })
})
