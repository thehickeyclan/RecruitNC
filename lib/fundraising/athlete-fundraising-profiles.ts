...
export async function resolveFundraisingAthletePublic(
  ...
  const { data: profile, error } = await admin
    ...
    .select(
      "id, created_at, updated_at, athlete_id, slug, bio, photo_url, is_active, campaign_goal_cents, total_raised_cents, primary_fundraising_code, checkout_live, fundraising_video_url, fundraising_video_thumbnail_url",
    )
    .eq("slug", slug)
    .eq("is_active", true)
    .maybeSingle()
